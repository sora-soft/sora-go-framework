...
Connector SHALL 实现状态机：`Init(1) → Connecting(2) → Ready(4) → Stopping(5) → Stopped(6)`，任意状态可进入 `Error(100)`。状态迁移 SHALL 使用已有的 `utility.LifeCycle[T]`。
...
Connector 的 `Start`、`SendRaw`、`Disconnect` SHALL 接受 `context.Context` 并传播至 Transport 层。Connector SHALL 持有一个生命周期 context，`Disconnect` 时取消该 context。
