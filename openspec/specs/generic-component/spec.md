...
### 需求:泛型 GetComponentOf 函数
Runtime 包必须（MUST）提供包级泛型函数 `GetComponentOf[T ComponentImpl](name string) (*BaseComponent[T], error)`。此函数必须（MUST）在内部完成从 `Component` 到 `*BaseComponent[T]` 的类型断言，并对调用者隐藏此细节。
